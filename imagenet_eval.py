...
def validate(val_loader, model, criterion):
    with torch.no_grad():
        ...
        for i, (input, target) in enumerate(val_loader):
            ...
            # measure elapsed time
            batch_top1.append(top1.val)
            batch_top5.append(top5.val)
            batch_time.update(time.time() - end)
            end = time.time()
        ...
        batch_top1 = np.array(batch_top1)
        batch_top5 = np.array(batch_top5)
        np.save(args.arch+'_top1.npy',batch_top1)
        np.save(args.arch+'_top5.npy',batch_top5)
